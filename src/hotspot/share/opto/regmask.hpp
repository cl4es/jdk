...
static unsigned int find_lowest_bit(uint64_t mask) {
  return count_trailing_zeros(mask);
}
// Find highest 1, undefined if empty/0
static unsigned int find_highest_bit(uint64_t mask) {
...
#   undef BODY
    _lwm = 0;
    _hwm = _RM_SIZE - 1;
    while (_hwm > 0 && _RM_UL[_hwm] == 0) _hwm--;
    while ((_lwm < _hwm) && _RM_UL[_lwm] == 0) _lwm++;
  ...
  RegMask(RegMask *rm) {
    ...
    for (unsigned i = 0; i < _RM_SIZE; i++) {
      _RM_UL[i] = rm->_RM_UL[i];
    }
    assert(valid_watermarks(), "post-condition");
  }

  // Construct an empty mask
  RegMask() : _RM_UL(), _lwm(_RM_SIZE - 1), _hwm(0) {}
  ...
  bool Member(OptoReg::Name reg) const {
    ...
    return _RM_UL[r >> _LogWordBits] & (1ULL <<(r & (_WordBits - 1U)));
  }
  ...
  bool is_AllStack() const { return _RM_UL[_RM_SIZE - 1U] >> (_WordBits - 1U); }
  ...
  void Clear() {
    _lwm = _RM_SIZE - 1;
    _hwm = 0;
    memset(_RM_UL, 0, sizeof(uint64_t)*_RM_SIZE);
    assert(valid_watermarks(), "sanity");
  }
  ...
  void Set_All() {
    _lwm = 0;
    _hwm = _RM_SIZE - 1;
    memset(_RM_UL, 0xFF, sizeof(uint64_t)*_RM_SIZE);
    assert(valid_watermarks(), "sanity");
  }
  ...
  void Insert(OptoReg::Name reg) {
    ...
    _RM_UL[index] |= (1ULL<<(r & (_WordBits - 1U)));
    assert(valid_watermarks(), "post-condition");
  }
  ...
  void Remove(OptoReg::Name reg) {
    assert(reg < CHUNK_SIZE, "");
    unsigned r = (unsigned)reg;
    _RM_UL[r >> _LogWordBits] &= ~(1ULL << (r & (_WordBits-1U)));
  }
