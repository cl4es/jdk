...
final class BootstrapMethodInvoker {
    ...
    static <T> T invoke(Class<T> resultType,
        ...
        try {
            ...
            if (info == null) {
                ...
                if (argv.length == 3 && isLambdaMetafactoryIndyBSM(bsmType)) {
                    result = (CallSite)bootstrapMethod
                            .invokeExact(caller, name, (MethodType)type, (MethodType)argv[0],
                                    (MethodHandle)argv[1], (MethodType)argv[2]);
                } else if (argv.length >= 1 && isStringConcatFactoryBSM(bsmType)) {
                ...
                } else if (argv.length == 3 && isProxyGetMethodBSM(bsmType)) {
    ...
    /**
     * @return true iff the BSM method type exactly matches the type required for the
     *         getMethod condy generated by {@link java.lang.reflect.ProxyGenerator}
     */
    private static boolean isProxyGetMethodBSM(MethodType bsmType) {
        return bsmType == PROXY_GET_METHOD_MT;
    }
